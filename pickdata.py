...
def date_inscription(pseudo:str):
    ...
    """
    curseur.execute("SELECT date_inscription FROM Joueur WHERE pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(row[0][0])
...
def set_grille_profil(idgrille:int, board:list, diff:float=0, solution:list=None):
    ...
    if solution == None :
        solution = "NULL"
    curseur.execute("INSERT INTO grille VALUES ('"+str(idgrille)+"','"+str(board)+"','"+str(solution)+"','"+str(diff)+"')")
    db.commit()
...
def get_nbparties(pseudo:str):
    ...
    """
    curseur.execute("SELECT nbparties FROM statistiques, joueur WHERE statistiques.idJoueur=joueur.idJoueur AND pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(list(row[0][0]))
...
def get_victoire(pseudo:str):
    ...
    """
    curseur.execute("SELECT victoires FROM statistiques, joueur WHERE statistiques.idJoueur=joueur.idJoueur AND pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(row[0][0])
...
def get_defaite(pseudo:str):
    ...
    """
    curseur.execute("SELECT défaites FROM statistiques, joueur WHERE statistiques.idJoueur=joueur.idJoueur AND pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(row[0][0])
...
def get_nul(pseudo:str):
    ...
    """
    curseur.execute("SELECT nuls FROM statistiques, joueur WHERE statistiques.idJoueur=joueur.idJoueur AND pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(row[0][0])
...
def calc_ratio(pseudo:str):
    ...
    ratio = v/d
    curseur.execute("INSERT INTO statistiques(ratiovd) VALUES ("+str(ratio)+")")

def get_ratio(pseudo:str):
    """
    Getter du ratio victoire/défaite
    """
    curseur.execute("SELECT ratiovd FROM statistiques, joueur WHERE statistiques.idJoueur=joueur.idJoueur AND pseudo='"+str(pseudo)+"'")
    row = curseur.fetchall()
    print(row[0][0])
...
def set_user(pseudo:str, mdp:str):
    ...
    if verif_pseudo_unique(pseudo, mdp)==True :     #vérifie l'unicité du pseudo
        ...
        curseur.execute("INSERT INTO joueur(idJoueur, pseudo, mdp, date_inscription) VALUES (NULL, '"+str(pseudo)+"', '"+str(mdp)+"', '"+str(date)+"')")
        db.commit()         #Envoie à la BDD le profil du joueur
        id = curseur.execute("SELECT idJoueur FROM joueur WHERE pseudo='"+str(pseudo)+"'")
        row = curseur.fetchall()        #Prend le nouvel id du joueur
        curseur.execute("INSERT INTO statistiques VALUES ('"+str(row[0][0])+"', '0', '0', '0', '0', '0', NULL)")
...
get_mdp("Bg3")
get_defaite("Bg7")
liste = [None, None, 9, None, None, None, None, 1, 4], [1, None, 4, None, None, None, 6, 5, 2], [None, 3, 5, None, 2, None, None, None, 8], [None, 5, 7, None, 3, 1, 2, 6, 9], [None, 6, None, None, None, None, None, 3, None], [3, None, None, None, None, 2, None, 8, None], [None, None, 8, None, 9, None, None, 2, 1], [9, None, None, 7, None, None, 8, None, None], [None, 1, None, None, None, 8, None, None, None]
...
#get_mdp("Benoit")
print(curseur.execute("SELECT pseudo, mdp FROM Joueur WHERE pseudo='Bg7' AND mdp='mdpbien'"))
db.close()
